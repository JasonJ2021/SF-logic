...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n::t => if (odd n) then n::(oddmembers t)
            else oddmembers t
end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => O
  | n::t => if (eqb v n) then 1 + (count v t) else count v t
  end.
...
Fixpoint member (v : nat) (s : bag) : bool :=
  match s with
  | nil => false
  | n::t => if (eqb n v) then true else (member v t)
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | n::t => if (eqb n v) then t else (n::(remove_one v t))
end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | n::t => if (eqb n v) then remove_all v t else (n::(remove_all v t))
end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | n::t => if (leb  (count n s1) (count n s2) ) then included t s2 else false
end.
...
Theorem add_inc_count : forall n b  , 
  length (add n b ) = 1 + (length b).
Proof.
...
Proof.
  intros l1 l2. induction l1 as [| n t IHl'].
...
Proof.
  intros l1 l2 l3 l4. induction l1 as [| n t IHl'].
...
Proof.  
  intros l1 l2. induction l1 as [| n t LHl'].
  - simpl. reflexivity.
  - destruct n as [| n' ].
    {
      simpl. rewrite -> LHl'.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | nil , _ => false
  | _  , nil => false
  | n1::t1 , n2::t2 => if (eqb n1 n2) then (eqblist t1 t2) else false
  end.
...
Proof.
  intros s. induction s as [| n t LHb'].
  - simpl. reflexivity.
  - destruct n as [|n'].
   {
...
Proof.
  ...
  intros n1 n2.
  intros h.
  rewrite -> h2. rewrite <-h. rewrite<- h2.
  reflexivity.
Qed.
(** [] *)
...
Proof.
  intros l1 l2.
  intros rev.
  rewrite <- rev_involutive. rewrite <- rev.
